...
1. **作成**: `chat-worktree` スキルを使用し、リポジトリルート内の **`.worktrees/`** ディレクトリを作業環境として使用してください。
2. **命名規則**: ワークツリーおよびブランチ名は、`[conversation-id]_[summary]`（例: `fee2607c..._fix-vscode-settings`）形式としてください。
3. **絶対パスの遵守**: ワークツリー作成後は、ベースリポジトリのパスではなく、**ワークツリーの絶対パス**を全てのツール呼び出しで使用してください。
4. **コーパス制約への対応**: ワークツリーがコーパス外にあるために `run_command` 等が失敗する場合、パスをコーパス内に変更するか、ツール呼び出しが可能なパスを正しく選択してください。
5. **コミット・PR**: 全ての変更はワークツリー内のブランチに対して行い、PR作成までその環境を維持してください。
